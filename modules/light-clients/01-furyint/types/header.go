...
// GetHeight returns the current height. It returns 0 if the furyint
// header is nil.
...
func (h Header) ValidateBasic() error {
	if h.SignedHeader == nil {
		return sdkerrors.Wrap(clienttypes.ErrInvalidHeader, "furyint signed header cannot be nil")
	}
	if h.Header == nil {
		return sdkerrors.Wrap(clienttypes.ErrInvalidHeader, "furyint header cannot be nil")
	}
	tmSignedHeader, err := tmtypes.SignedHeaderFromProto(h.SignedHeader)
	if err != nil {
		return sdkerrors.Wrap(err, "header is not a furyint header")
	}
	...
	if err != nil {
		return sdkerrors.Wrap(err, "validator set is not furyint validator set")
	}
...
func (h Header) ValidateCommit() (err error) {
	...
	if err != nil {
		return sdkerrors.Wrap(err, "commit is not furyint commit type")
	}
	tmValset, err := tmtypes.ValidatorSetFromProto(h.ValidatorSet)
	if err != nil {
		return sdkerrors.Wrap(err, "validator set is not furyint validator set type")
	}
