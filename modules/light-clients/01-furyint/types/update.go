...
// - the client or header provided are not parseable to furyint types
...
// Dymint client validity checking uses the bisection algorithm described
...
// checkValidity checks if the Dymint header is valid.
...
func checkValidity(
	...
	if err != nil {
		return sdkerrors.Wrap(err, "signed header in not furyint signed header type")
	}

	_, err = tmtypes.ValidatorSetFromProto(header.ValidatorSet)
	if err != nil {
		return sdkerrors.Wrap(err, "validator set in not furyint validator set type")
	}
