...
func (suite *DymintTestSuite) TestStatus() {
	...
	for _, tc := range testCases {
		...
		status := clientState.Status(furyintCounterpartyChain.GetContext(), clientStore, furyintCounterpartyChain.App.AppCodec())
...
func (suite *DymintTestSuite) TestInitialize() {
	...
	for _, tc := range testCases {
		err := clientState.Initialize(furyintCounterpartyChain.GetContext(), furyintCounterpartyChain.Codec, store, tc.consensusState)
		if tc.expPass {
...
func (suite *DymintTestSuite) TestVerifyConnectionState() {
	var (
		...
		prefix                               commitmenttypes.MerklePrefix
		furyintChain, furyintCounterpartyChain *ibctesting.TestChain
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			var ok bool
			clientStateI := furyintCounterpartyChain.GetClientState(endpoint1.ClientID)
			clientState, ok = clientStateI.(*types.ClientState)
			suite.Require().True(ok)

			prefix = furyintChain.GetPrefix()

			// make connection proof
			connectionKey := host.ConnectionKey(endpoint2.ConnectionID)
			proof, proofHeight = furyintChain.QueryProof(connectionKey)

			tc.malleate() // make changes as necessary

			store := furyintCounterpartyChain.App.GetIBCKeeper().ClientKeeper.ClientStore(furyintCounterpartyChain.GetContext(), endpoint1.ClientID)

			err := clientState.VerifyConnectionState(
				store, furyintCounterpartyChain.Codec, proofHeight, &prefix, proof, endpoint2.ConnectionID, connection,
			)
...
func (suite *DymintTestSuite) TestVerifyChannelState() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			proof, proofHeight = furyintChain.QueryProof(channelKey)
			...
			err := clientState.VerifyChannelState(
				store, furyintCounterpartyChain.Codec, proofHeight, &prefix, proof,
...
func (suite *DymintTestSuite) TestVerifyPacketCommitment() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			ctx := furyintCounterpartyChain.GetContext()
			store := furyintCounterpartyChain.App.GetIBCKeeper().ClientKeeper.ClientStore(ctx, endpoint1.ClientID)

			commitment := channeltypes.CommitPacket(furyintCounterpartyChain.App.GetIBCKeeper().Codec(), packet)
			err = clientState.VerifyPacketCommitment(
				ctx, store, furyintCounterpartyChain.Codec, proofHeight, delayTimePeriod, delayBlockPeriod, &prefix, proof,
...
func (suite *DymintTestSuite) TestVerifyPacketAcknowledgement() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			proof, proofHeight = furyintChain.QueryProof(acknowledgementKey)
...
func (suite *DymintTestSuite) TestVerifyNextSeqRecv() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			proof, proofHeight = furyintChain.QueryProof(nextSeqRecvKey)
